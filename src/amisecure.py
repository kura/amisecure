    ...
    #},
    {
        ...
        "tests": (
            #{
                ...
                #'display_value': True,
                #'test_name': "Timeout",
                #'additional_text': "Less than 5 seconds is good"
            #},
            #(
            ...
            #),
            #(
                #re.compile(r"[^a-z]SSLCipherSuite\s[a-z0-9\:\!\+]*?(?P<value>\+SSLv2)[a-z0-9\:\!\+]*?", re.IGNORECASE),
                #("equal_to", ("!SSLv2", "")), "Found",
                #"SSLv2 cipher", "Always disable, SSLv2 is not secure"
            #),
            #(
                #re.compile(r"[^a-z]SSLProtocol\s[a-z0-9\s]*?(?P<value>[^\-]SSLv2)[a-z0-9\s]*?", re.IGNORECASE),
                #("equal_to", ""), "Found",
                #"SSLv2 protocol", "Always disable, SSLv2 is not secure"
            #),
            #(
            ...
            #),
            {
                ...
                'secure_values': (""),
                'display_value': False,
                'display_text': {'match': "Found", 'no_match': "Not Found"},
...
def check_value(test, content):
    print test
    regex = test['regex']
    if regex.search(content):
        value = regex.findall(content)[-1]
        if globals()[test['method']](value, test['secure_values']):
            colour = "green"
            display = "%s (secure)" % (value)
            globals()['TOTAL_SECURE'] += 1
        else:
            colour = "red"
            display = "%s (not secure)" % (value)
            globals()['TOTAL_UNSECURE'] += 1
    elif test['secure_on_empty'] is True:
        colour = "green"
        display = "not found (secure)"
        globals()['TOTAL_SECURE'] += 1
    else:
        colour = "yellow"
        display = "unknown"
        globals()['TOTAL_UNKNOWN'] += 1
    write_to_shell(test['test_name'], test['additional_text'], display, colour)
...
for system in config_checks:
    ...
    for test in system['tests']:
        globals()[system['check_function']](test, content)
